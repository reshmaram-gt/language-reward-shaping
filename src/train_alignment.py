...
with tqdm(dataloader, unit="batch") as pbar:
    ...
    for batch in pbar:
        ...
        batch_accuray = torch.sum(torch.where(output > 0.5, 1, 0).reshape(-1).to("cuda") == batch["label"].reshape(-1).to("cuda")) / 64
...
print("Dev accuracy: ", compute_metrics(dev_dataloader, text_tokenizer, text_encoder, model))